...
describe('sql.functions Test', function() {
  ...
  runTest("approxCountDistinct", ["name", 5], "APPROXIMATE COUNT(DISTINCT name)");
  runTest("approxCountDistinct", function(){return [dataFrame.col("name")]}, "APPROXIMATE COUNT(DISTINCT name)");
  runTest("avg", ["name"], "avg(name)");
  runTest("count", ["name"], "count(name)");
  runTest("countDistinct", function(){return [dataFrame.col("name"), dataFrame.col("age")]}, "COUNT(DISTINCT name,age)");
  ...
  runTest("sumDistinct", ["name"], "SUM(DISTINCT name)");
