...
function FloatRDD() {
  if (arguments.length == 2) {
    ...
    var srdd = arguments[0];

    this.kernelP = srdd.kernelP;

    var templateStr = 'var {{refId}} = new FloatRDD({{srdd}});';

    this.refIdP = Utils.evaluate(this.kernelP, FloatRDD, templateStr, {srdd: Utils.prepForReplacement(srdd)}, true);
  }
}
...
FloatRDD.fromRDD = function(rdd) {
  var templateStr = 'var {{refId}} = FloatRDD.fromRDD({{rdd}});';

  return Utils.evaluate(rdd.kernelP, FloatRDD, templateStr, {rdd: Utils.prepForReplacement(rdd)});
};
