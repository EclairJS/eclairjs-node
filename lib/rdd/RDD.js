...
RDD.prototype.subtract = function(other, numPartitions, p) {
  ...
  if (p) {
    templateStr = 'var {{refId}} = {{inRefId}}.subtract({{other}}, {{numPartitions}}, {{p}});';
  } else if (numPartitions) {
    templateStr = 'var {{refId}} = {{inRefId}}.subtract({{other}}, {{numPartitions}});';
  } else {
    templateStr = 'var {{refId}} = {{inRefId}}.subtract({{other}});';
  }

  return Utils.generateAssignment(this, RDD, templateStr, {other: Utils.prepForReplacement(other), numPartitions: numPartitions, p: Utils.prepForReplacement(p)});
};
...
RDD.prototype.zipWithIndex = function() {
  var templateStr = 'var {{refId}} = {{inRefId}}.zipWithIndex();';
