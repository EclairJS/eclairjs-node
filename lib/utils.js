...
Utils.generate = function(args) {
  ...
  var refIdP = new Promise(function(resolve, reject) {
    Promise.all(promises).then(function(values) {
      ...
      if (executionType == EXECUTION.RESULT_TYPE) {
  ...
  // handle target
  if (executionType == EXECUTION.RESULT_TYPE || executionType === EXECUTION.RESULT_ARRAY_TYPE || executionType == EXECUTION.VOID_TYPE) {
    return refIdP;
  } else if (executionType == EXECUTION.ASSIGNMENT_TYPE) {
      ...
      args.forEach(function (arg, i) {
        ...
        if (arg.type == 'string') {
          ...
          promises.push(new Promise(function (resolve, reject) {
            Utils.serializeLambda(arg.value).then(function (result) {
              resolve(result);
            }).catch(reject);
          }));
          ...
          promises.push(new Promise(function (resolve, reject) {
            arg.value.refIdP.then(function (result) {
              resolve(result);
            }).catch(reject);
          }));
...
Utils.wrapArray = function(arr, sparkType) {
  ...
  if (arr && arr.length > 0) {
    var type;
    if (sparkType && arr[0] instanceof sparkType) {
      type = sparkType;
    } else {
    ...
    arr.forEach(function(a) {
      wrapArr.push({value: a, type: type})
    });
  }
