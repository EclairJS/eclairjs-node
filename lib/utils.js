...
Utils.generate = function(args) {
  ...
  var refIdP = new Promise(function(resolve, reject) {
    Promise.all(promises).then(function(values) {
      ...
      /*if (target.moduleLocation) {
        var targetClassName = typeof target == 'object' ? target.constructor.name : target.name;

        if (!moduleRequireCache[target.moduleLocation]) {
          code += 'var '+targetClassName+' = require(\'' + target.moduleLocation + '\');\n';
          moduleRequireCache[target.moduleLocation] = true;
        }
      }*/
