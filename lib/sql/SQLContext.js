...
SQLContext.prototype.read = function() {
  var refId = protocol.genVariable('dataFrameReader');
  var self = this;

  return new DataFrameReader(this.context.kernel, new Promise(function(resolve, reject) {
    Promise.all([self.context.kernel, self.sqlContextP]).then(function(values) {
      var kernel = values[0];
      var sqlContextId = values[1];

      var templateStr = 'var {{refId}} = {{sqlContextId}}.read();';
      var code = Utils.processTemplate(templateStr, {refId: refId, sqlContextId: sqlContextId});
