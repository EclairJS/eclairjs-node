...
DataFrame.prototype.except = function(otherDataFrame) {
  ...
  return new DataFrame(this.kernelP, new Promise(function(resolve, reject) {
    Promise.all([self.kernelP, self.refIdP, otherDataFrame.refIdP]).then(function(values) {
      var kernel = values[0];
      var dataFrameId = values[1];
      var otherDataFrameId = values[2];

      var templateStr = 'var {{refId}} = {{inRefId}}.except({{otherDataFrameId}});';
      var code = Utils.processTemplate(templateStr, {refId: refId, inRefId: dataFrameId, otherDataFrameId: otherDataFrameId});
...
DataFrame.prototype.explain = function(extended) {
  ...
  return new Promise(function(resolve, reject) {
    Promise.all([self.kernelP, self.refIdP]).then(function(values) {
      ...
      var templateStr = '{{dataFrameId}}.explain({{extended}});';
      var code = Utils.processTemplate(templateStr, {dataFrameId: dataFrameId, extended: extended ? extended : ""});
...
DataFrame.prototype.first = function() {
  ...
  return new Row(this.kernelP, new Promise(function(resolve, reject) {
    Promise.all([self.kernelP, self.refIdP]).then(function(values) {
      ...
      var templateStr = 'var {{refId}} = {{dataFrameId}}.first();';
...
DataFrame.prototype.foreach = function(func) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    Promise.all([self.kernelP, self.refIdP, udfP]).then(function(values) {
      ...
      var templateStr = '{{dataFrameId}}.foreach({{udf}});';
      var code = Utils.processTemplate(templateStr, {dataFrameId: dataFrameId, udf: udf});
...
DataFrame.prototype.show = function() {
  ...
  return new Promise(function(resolve, reject) {
    Promise.all([self.kernelP, self.refIdP]).then(function(values) {
      ...
      var templateStr = '{{dataFrameId}}.show();';
      var code = Utils.processTemplate(templateStr, {dataFrameId: dataFrameId});
...
DataFrame.prototype.where = function(condition) {
  ...
  return new DataFrame(this.kernelP, new Promise(function(resolve, reject) {
    ...
    Promise.all(promises).then(function(values) {
      ...
      // diffefilrent template for string or object
