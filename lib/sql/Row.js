...
function areTwoRowsEqual(row1, row2) {
  var isEqual = row1 instanceof Row && row2 instanceof Row && deepEquals(row1.schema, row2.schema) && deepEquals(row1.values, row2.values);
...
Row.prototype.anyNull = function() {
  if (this.values) {
    var anyNull = this.values.some(function(item) {
      return item === null;
    });
...
Row.prototype.copy = function() {
  if (this.values) {
    return new Row(this.values, this.schema);
  } else {
    var args = {
      target: this,
      method: 'copy',
...
Row.prototype._fieldIndex = function(name) {
  ...
  this.schema.fields.some(function(field, i) {
    if (field.name === name) {
      index = i;
      return true;
    } else {
...
Row.prototype.getBoolean = function(index) {
  if (this.values) {
    if (this.values.length > index) {
      var type = this.schema.fields[index];
  ...
  if (this.values) {
    if (this.values.length > index) {
      ...
      if (type.dataType === 'date') {
        return new SqlDate(this.values[index]);
      } else {
  ...
  if (this.values) {
    if (this.values.length > index) {
      ...
      if (type.dataType === 'string') {
        ...
        throw new Error('the type for index ' + index + ' is not a Strubg');
      }
    } else {
      throw new Error('index is out of bounds');
    }
