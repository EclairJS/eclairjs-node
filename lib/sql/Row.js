...
Row.prototype.getDate = function(index) {
  ...
  var refId = protocol.genVariable('sqlDate');
  var self = this;

  var SqlDate = require('./SqlDate.js');

  return new SqlDate(this.kernelP, new Promise(function(resolve, reject) {

    function _resolve(val) {
      // parse stringified date here and return as date
      console.log('date: ',val.toString());
      console.log('date: ',val.valueOf());
    ...
    Promise.all([self.refIdP, self.kernelP]).then(function(values) {
      ...
      var code = Utils.processTemplate(templateStr, {refId: refId, inRefId: inRefId, index: index});
      //protocol.verifyResult(kernel.execute({code: code}), _resolve, reject);
      protocol.verifyAssign(kernel.execute({code: code, silent: false}),
        _resolve,
        reject,
