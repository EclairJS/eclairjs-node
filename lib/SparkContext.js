...
function SparkContext(master, name) {
  if (arguments.length == 2 && arguments[0] instanceof Promise) {
    ...
    if (arguments.length == 2) {
      kernel.createKernelSession(arguments[1]).then(kernelPResolve).catch(kernelPReject);
    } else {
      ...
      gKernelP.then(function(kernel) {
        ...
        Utils.generateConstructor(args).then(function(refId) {
          ...
          Utils.generate(args).then(function(version) {
            if (version === 'EclairJS-nashorn 0.6-SNAPSHOT Spark 1.6.0' ||
                version === 'EclairJS-nashorn 0.5 Spark 1.6.0' ||
                version === 'EclairJS-nashorn 0.4 Spark 1.6.0') {
              // correct version
              resolve(kernel);
            } else {
