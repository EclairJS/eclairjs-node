 ...
 * @param {string} name Optional Name of the accumulator for display in Spark's web UI.
 ...
 * @param {string} (Optional) name of  the accumulator for display in Spark's web UI.
 * @returns {Accumulator}
 */
SparkContext.prototype.accumulator = function(initialValue, name) {
  var Accumulator = require('./Accumulator.js');

  var templateStr = name ? 'var {{refId}} = jsc.accumulator({{initialValue}}, {{name}});' : 'var {{refId}} = jsc.accumulator({{initialValue}});';

  return Utils.evaluate(this.kernelP, Accumulator, templateStr, {initialValue: Utils.prepForReplacement(initialValue), name: Utils.prepForReplacement(name)});
};
...
SparkContext.prototype.stop = function() {
  ...
  return Utils.generateVoidPromise(this.kernel, templateStr);
  */
