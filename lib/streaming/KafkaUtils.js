...
KafkaUtils.createStream = function(ssc, zk, consumer_group, topic) {
  ...
  return new DStream(ssc.context.kernelP, new Promise(function(resolve, reject) {
    Promise.all([ssc.context.kernelP, ssc.streamingContextP]).then(function(values) {
      ...
      var code = Utils.processTemplate(templateStr, 
                                       {refId: refId, 
                                        ...
                                        topic: topic});
      console.log("KafkaUtils.createStream code is:");
      console.log(code);
      ...
      console.log('failed to createStream');
      console.log(err);
      reject(err)
    })
  }))
