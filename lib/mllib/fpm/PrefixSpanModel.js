...
function PrefixSpanModel() {
  if (arguments.length == 2) {
    ...
    var templateStr = 'var {{refId}} = new PrefixSpanModel({{freqSequences}});';

    this.refIdP = Utils.evaluate(gKernelP, PrefixSpanModel, templateStr, {freqSequences: Utils.prepForReplacement(arguments[0])}, true);
  }
}

PrefixSpanModel.prototype.freqSequences = function() {
  var templateStr = 'var {{refId}} = {{inRefId}}.freqSequences();';
