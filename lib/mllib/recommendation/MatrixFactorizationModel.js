...
MatrixFactorizationModel.prototype.predict = function(user, product) {
  ...
  if (product) {
    ...
    var templateStr = 'var {{refId}} = {{inRefId}}.predict({{usersProducts}});';
    return Utils.generateAssignment(this, RDD, templateStr, {usersProducts: Utils.prepForReplacement(user)});
  }
};
