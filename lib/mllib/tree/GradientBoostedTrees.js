...
function GradientBoostedTrees() {
  if (arguments.length == 2) {
    ...
    var templateStr = 'var {{refId}} = new GradientBoostedTrees({{boostingStrategy}});';

    this.refIdP = Utils.evaluate(gKernelP, GradientBoostedTrees, templateStr, {boostingStrategy: Utils.prepForReplacement(arguments[0])}, true);
  }
}
...
GradientBoostedTrees.prototype.run = function(input) {
  ...
  return Utils.generateAssignment(this, GradientBoostedTreesModel, templateStr, {input: Utils.prepForReplacement(input)});
};
...
GradientBoostedTrees.prototype.runWithValidation = function(input,validationInput) {
  var templateStr = 'var {{refId}} = {{inRefId}}.runWithValidation({{input}},{{validationInput}});';

  return Utils.generateAssignment(this, GradientBoostedTreesModel, templateStr, {input: Utils.prepForReplacement(input), validationInput: Utils.prepForReplacement(validationInput)});
};
...
GradientBoostedTrees.train = function(input, boostingStrategy) {
  var templateStr = 'var {{refId}} = GradientBoostedTrees.train({{input}}, {{boostingStrategy}});';

  return Utils.evaluate(gKernelP, GradientBoostedTreesModel, templateStr, {input: Utils.prepForReplacement(input), boostingStrategy: Utils.prepForReplacement(boostingStrategy)});
};
