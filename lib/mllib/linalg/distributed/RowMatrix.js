...
function RowMatrix() {
  if (arguments.length == 2) {
    ...
    var templateStr = arguments.length == 3 ? 'var {{refId}} = new RowMatrix({{rows}}, {{nRows}}, {{nCols}});' :  'var {{refId}} = new RowMatrix({{rows}});';

    return Utils.evaluate(gKernelP, RowMatrix, templateStr, {rows: Utils.prepForReplacement(arguments[0]), nRows: arguments[1], nCols: arguments[2]});
  }
}
 ...
 */
RowMatrix.prototype.numCols = function() {
 ...
 * @returns {Promise.<number>}
 */
RowMatrix.prototype.numRows = function() {
  throw "not implemented by ElairJS";
//
// function _resolve(result, resolve, reject) {
// 	var returnValue=parseInt(result)
...
RowMatrix.prototype.computeGramianMatrix = function() {
  throw "not implemented by ElairJS";
//
// var templateStr = 'var {{refId}} = {{inRefId}}.computeGramianMatrix();';
//
...
RowMatrix.prototype.computeSVD = function(k,computeU,rCond) {
  var templateStr = 'var {{refId}} = {{inRefId}}.computeSVD({{k}},{{computeU}},{{rCond}});';

  return Utils.generateAssignment(this, SingularValueDecomposition, templateStr, {k: k, computeU: computeU, rCond: rCond});
};
...
RowMatrix.prototype.computeCovariance = function() {
  throw "not implemented by ElairJS";
//
// var templateStr = 'var {{refId}} = {{inRefId}}.computeCovariance();';
//
// return Utils.generateAssignment(this, Matrix, templateStr );
};
...
RowMatrix.prototype.computePrincipalComponents = function(k) {
  var templateStr = 'var {{refId}} = {{inRefId}}.computePrincipalComponents({{k}});';

  return Utils.generateAssignment(this, Matrix, templateStr, {k: k});
};
...
RowMatrix.prototype.computeColumnSummaryStatistics = function() {
  throw "not implemented by ElairJS";
//
// var templateStr = 'var {{refId}} = {{inRefId}}.computeColumnSummaryStatistics();';
//
// return Utils.generateAssignment(this, MultivariateStatisticalSummary, templateStr );
};
...
RowMatrix.prototype.multiply = function(B) {
  var templateStr = 'var {{refId}} = {{inRefId}}.multiply({{B}});';

  return Utils.generateAssignment(this, RowMatrix, templateStr, {B: Utils.prepForReplacement(B)});
};
...
//
// var templateStr = 'var {{refId}} = {{inRefId}}.columnSimilarities({{threshold}});';
//
// return Utils.generateAssignment(this, CoordinateMatrix, templateStr , {threshold : threshold});
};
...
RowMatrix.prototype.tallSkinnyQR = function(computeQ) {
  throw "not implemented by ElairJS";
//
// var templateStr = 'var {{refId}} = {{inRefId}}.tallSkinnyQR({{computeQ}});';
//
// return Utils.generateAssignment(this, QRDecomposition, templateStr , {computeQ : computeQ});
};
...
RowMatrix.prototype.rows = function() {
  var templateStr = 'var {{refId}} = {{inRefId}}.rows();';
