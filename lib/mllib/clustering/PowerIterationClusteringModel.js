...
function PowerIterationClusteringModel() {
  if (arguments.length == 2 && arguments[0] instanceof Promise) {
    ...
    var templateStr = 'var {{refId}} = new PowerIterationClusteringModel({{k}}, {{assignments}});';

    this.refIdP = Utils.evaluate(gKernelP, PowerIterationClusteringModel, templateStr, {k: arguments[0], assignments: Utils.prepForReplacement(arguments[1])}, true);
  }
}
...
PowerIterationClusteringModel.prototype.assignments = function() {
  var templateStr = 'var {{refId}} = {{inRefId}}.assignments();';
