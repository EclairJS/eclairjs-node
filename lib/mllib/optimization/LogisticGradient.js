...
function LogisticGradient() {
  if (arguments.length == 2) {
    ...
    var templateStr = arguments[0] ? 'var {{refId}} = new LogisticGradient({{numClasses}});' : 'var {{refId}} = new LogisticGradient();';

    this.refIdP = Utils.evaluate(gKernelP, LogisticGradient, templateStr, {numClasses: arguments[0]}, true);
  }
}
