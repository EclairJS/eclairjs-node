...
function SVMWithSGD() {
  if (arguments.length == 2) {
    ...
    var templateStr = 'var {{refId}} = new SVMWithSGD();';
    this.refIdP = Utils.evaluate(gKernelP, SVMWithSGD, templateStr, null, true);
  }
}
...
SVMWithSGD.train = function (input, numIterations, stepSize, regParam, miniBatchFraction, initialWeights) {
  ...
  if (arguments.length == 6) {
    templateStr = 'var {{refId}} = SVMWithSGD.train({{input}},{{numIterations}},{{stepSize}},{{regParam}},{{miniBatchFraction}},{{initialWeights}});';
  } else if (arguments.length == 5) {
    templateStr = 'var {{refId}} = SVMWithSGD.train({{input}},{{numIterations}},{{stepSize}},{{regParam}},{{miniBatchFraction}});';
  } else if (arguments.length == 4) {
    templateStr = 'var {{refId}} = SVMWithSGD.train({{input}},{{numIterations}},{{stepSize}},{{regParam}});';
  } else {
    templateStr = 'var {{refId}} = SVMWithSGD.train({{input}},{{numIterations}});';
  }

  return Utils.evaluate(gKernelP, SVMModel, templateStr, {input: Utils.prepForReplacement(input), numIterations: numIterations, stepSize: stepSize, regParam: regParam, miniBatchFraction: miniBatchFraction, initialWeights: Utils.prepForReplacement(initialWeights)});
};
